...
vignette: >
  ...
  %\VignetteDepends{Spectra,BiocStyle,msdata,MetaboAnnotation,SummarizedExperiment,QFeatures}
...
and facilitate annotation of MS-based metabolomics data [@rainer_modular_2022].
...
`Spectra`). As an alternative, it would also be possible to use a `CompDb`
object representing a compound annotation database (defined in the
`r Biocpkg("CompoundDb")` package) with parameter `target`. See the
`matchSpectra` help page or section *Query against multiple reference databases*
...
conversion of these into a format suitable for import. To simplify this process,
the concept of *compound annotation sources* was introduced. These represent
references (links) to annotation resources and take care of retrieving the
annotation resources making integration of various annotation resources easier
for the end user. In addition, such *compound annotation sources* (object
extending `CompAnnotationSource`) can be combined to allow matching query
spectra against multiple reference libraries in a single call.
