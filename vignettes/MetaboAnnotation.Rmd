...
parameter object (in our case a `TargetMass2MzParam`) with the settings for the
matching. With the `TargetMass2MzParam`, the mass or target compounds get first
...
```{r}
parm <- TargetMass2MzParam(adducts = c("[M+H]+", "[M+Na]+"),
...
`DataFrame` with all rows in *query* the corresponding matches in *target*
along with the matching adduct (column `"adduct"`) and the difference in m/z
(column `"score"`). Note that if a row in *query* matches multiple elements in
...
See also the help page for `?Matched` for more details and information. In
addition to the matching of query m/z against target exact masses as described
above it would also be possible to match directly query m/z against target m/z
values by using the `MzParam` instead of the `TargetMass2MzParam`.


## Matching of m/z and retention time values

If expected retention time values were available for the target compounds, an
annotation with higher confidence could be performed with `matchMz` and a
`TargetMass2MzRtParam` parameter object. To illustrate this we randomly assign
...
keep the runtime of the vignette short) and rename column `"rtime"` to `"rt"`
(as retention times are expected to be provided in a column named `"rt"`).

```{r}
ms1_subset <- ms1_features[1:100, ]
colnames(ms1_subset) <- sub("rtime", "rt", colnames(ms1_subset))
```
...
```{r}
set.seed(123)
target_df$rt <- sample(ms1_subset$rt, nrow(target_df), replace = TRUE) + 2
```
...
`TargetMass2MzRtParam` which allows us to specify (as for the
`TargetMass2MzParam`) the expected adducts, the maximal acceptable m/z relative
...
difference of 10 seconds in retention times.

```{r}
parm <- TargetMass2MzRtParam(adducts = c("[M+H]+", "[M+Na]+"),
                             tolerance = 0.005, ppm = 0,
                             toleranceRt = 10)
