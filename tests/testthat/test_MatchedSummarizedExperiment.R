...
test_that("keepMatches,Matched works", {
  ...
  expect_error(keepMatches(mo, idxs = c(1, 10)), "out of bounds")
  ## no index : every match is removed
  idxs <- integer(0)
  mosub <- keepMatches(mo, idxs = idxs)
  ...
  mosub <- keepMatches(mo, queryValues = queryValues + 100, 
...
test_that("dropMatches,MatchedSummarizedExperiment works", {
  #### query and target data.frames
  mo <- MatchedSummarizedExperiment(
    q1, t1, matches = data.frame(query_idx = c(1L, 2L, 2L, 2L, 5L),
                                 target_idx = c(2L, 2L, 3L, 4L, 5L),
                                 score = seq(0.5, 0.9, by = 0.1)))
  ## out of bounds indexes
  expect_error(dropMatches(mo, idxs = c(1, 10)), "out of bounds")
  ## all indexes : every match is removed
  idxs <- seq_len(nrow(mo@matches))
  ...
  expect_equal(target(mosub), target(mo))
  ## no matches corresponding to the input values
  mosub <- dropMatches(mo, queryValues = queryValues + 100, 
                       targetValues = targetValues + 100, 
                       queryColname = "col1", targetColname = "col2")
